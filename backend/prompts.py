...
- Extract ALL source citations, references, URLs, and attributions from the raw response
- Put the main formatted content in the "main_response" field
- Put ALL source information in the "sources" field as a list
- Include source titles, URLs, publication dates, and any other attribution details
- If the raw response has "Sources:" sections, extract each source as a separate list item
- If sources are mentioned inline (like "According to [Source]..."), extract those too
- Never lose any source information during formatting
...
Preserve all factual information and source attributions exactly as presented in the raw response.
"""
